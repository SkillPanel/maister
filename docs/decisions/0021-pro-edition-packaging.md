...
Separately, cutting a `contracts-v*` tag is the pro repository's release trigger, and an accidental tag push must not publish anything while the register and runner inside the pro repository are still new. A tag-protection ruleset was the first choice for holding that job shut — protect the `contracts-v*` pattern at the repository-settings level, before a workflow run is even scheduled — but that setting is not available to the pro repository. In its place, `contracts.yml` takes a **job-level `if:` guard** on a repository variable, left unset through this wave, so an accidental tag still starts the job but the job itself skips immediately. The guard is visible in the workflow file a reader already opens, rather than hidden in a settings page.
