    ...
    const release = await resolveReleaseMetadata({
      ...
      requestMetadata: async ({ url, credential }) => {
        try {
          ...
          if (error?.kind === "E_LAUNCHER_TRANSPORT_HTTP" && (
            [401, 403, 404, 408, 429].includes(error?.details?.status)
            || (Number.isInteger(error?.details?.status) && error.details.status >= 500 && error.details.status <= 599)
          )) {
