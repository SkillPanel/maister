...
async function streamingTreeHash(root, { ignore = () => false, expectedModes = null } = {}) {
  const entries = [];
  async function visit(current, relative) {
    ...
    for (const name of names) {
      ...
      const mode = (expectedModes?.get(childRelative) ?? (stat.mode & 0o7777)).toString(8).padStart(4, "0");
...
function requireArchiveEvidence({ target, asset, plan, sidecars }) {
  ...
  const directAsset = asset?.direct === true;
  if (!expectedName
    || (!directAsset && (!Number.isSafeInteger(asset?.id) || asset.id <= 0))
...
export async function validateExtractedRelease({
  ...
  const archiveModes = new Map(plan.entries.map((entry) => [entry.path, entry.mode]));
  // tar may materialize an implicit parent directory that was not serialized
  // as an archive entry (currently the top-level `plugins` directory). Give
  // those parents the canonical release directory mode when hashing the
  // extracted tree, including on Windows where stat.mode is not POSIX.
  for (const entry of plan.entries) {
    for (let parent = path.posix.dirname(entry.path); parent !== "."; parent = path.posix.dirname(parent)) {
      archiveModes.set(parent, archiveModes.get(parent) ?? 0o755);
    }
  }
  const observedContentHash = await streamingTreeHash(root, {
    ignore: (relative) => relative === "plugins/maister/.maister-source.json",
    expectedModes: archiveModes,
  });
  ...
  const portablePrefix = "plugins/maister/common/";
  const portableModes = new Map(plan.entries
    .filter((entry) => entry.path.startsWith(portablePrefix))
    .map((entry) => [entry.path.slice(portablePrefix.length), entry.mode]));
  const portableCoreDigest = await streamingTreeHash(portableCoreRoot, { expectedModes: portableModes });
