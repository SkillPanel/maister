...
AskUserQuestion — "Problem space explored." Read `orchestrator.next_phase` from `orchestrator-state.yml` (a phase id). If it is `phase-4`, prepend "Skipping persona exploration (enhancement scope). " Ask "Continue to [title of that phase]?"
...
AskUserQuestion — "Specification complete." Read `orchestrator.next_phase` from `orchestrator-state.yml` (a phase id). If it is `phase-8`, prepend "No UI prototyping needed (backend-focused design). " Ask "Continue to [title of that phase]?"
