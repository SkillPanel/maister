...
The performance and migration context shapes were derived from their orchestrators' own definitions, because no real run at or above the floor existed when the contract was written. `performance` is now pinned by a run sampled at 2.3.0-beta.15, and the shape did not change; its synthetic fixture stays beside it for the field values a run stopped at its first gate leaves null. `migration` is still synthetic-only, and not for want of a run: a real one was produced the same day and **the exactly-one rule above refused it**, because both orchestrators' prose directs a phase to write `task_context.clarifications_resolved` beside their own context block. Every fixture that carries `task_context` today is a development run, where it *is* the per-workflow block, so nothing here had caught the collision. The rule and the prose disagree and one of them has to move; until that is decided, neither is changed.
