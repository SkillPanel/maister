...
phases in `references/development-twin.md`. Both produce the same task directory; only the
interpreter differs.
...
- **A non-empty value** — or no script runtime to read it with — selects the prose twin. Read
  `references/development-twin.md` and run the phases in it exactly as written, with nothing else
  changed by the presence of this branch.
- **No value, or an empty one — the default** — hand the run over. Invoke the
  `maister:workflow-engine` skill with the Skill tool, naming the workflow `builtin:development`
...
Full framework rule: `../orchestrator-framework/references/orchestrator-patterns.md` § 2 and § 2.1. The questions this workflow asks *inside* a phase follow the same driver: § 2.2 states what a non-terminal run takes instead of asking, and every one of them names its default where its phase is defined — in the workflow definition on the engine path, in `references/development-twin.md` on the prose path.
