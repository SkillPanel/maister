...
export function classifyState(projectRoot) {
	...
	const complete =
		...
		fs.existsSync(paths.docsIndex) &&
		fs.existsSync(paths.techStack) &&
		fs.existsSync(path.join(paths.standards, "README.md"));
...
export function validateCandidateConfig(configPath, { enabled = "off" } = {}) {
	if (!fs.existsSync(configPath) || !fs.statSync(configPath).isFile())
		fail(`candidate config does not exist: ${configPath}`, "E_CONFIG");
	if (enabled !== "on" && enabled !== "off")
		fail("Advisor intent must be on or off", "E_ARGUMENT");
	...
	try {
		...
		execFileSync(SCRIPT_PATH, ["candidate", source, enabled, candidate], {
			encoding: "utf8",
...
function validateArtifactInputs(options, advisor) {
	if (options.configText !== undefined)
		validateText(options.configText, advisor);
	for (const relative of Object.keys(options.artifacts ?? {})) {
		if (!relative || relative.startsWith("/") || relative.includes(".."))
			fail(`unsafe artifact path: ${relative}`, "E_ARGUMENT");
		if (typeof options.artifacts[relative] !== "string")
			fail(`artifact content must be text: ${relative}`, "E_ARGUMENT");
	}
}
...
function resolveAction(classification, options) {
	...
	if (!selected)
		fail(
			"an action is required for an existing .maister state",
			"E_ACTION_REQUIRED",
		);
	const aliases = {
		backup: "backup",
	...
	if (!Object.hasOwn(aliases, selected))
		fail(`unsupported init action: ${selected}`, "E_ARGUMENT");
	if (
		classification === "fresh" &&
		selected !== "create" &&
		selected !== "cancel"
	)
		fail(`unsupported fresh-state action: ${selected}`, "E_ARGUMENT");
	if (
		selected === "resume" &&
		!fs.existsSync(
			path.join(
				artifactPaths(path.resolve(options.projectRoot ?? process.cwd())).root,
...
export function preflight(options = {}) {
	...
	if (!fs.existsSync(projectRoot) || !fs.statSync(projectRoot).isDirectory())
		fail("project root must be a directory", "E_ARGUMENT");
	...
	if (fs.existsSync(paths.config))
		validateCandidateConfig(paths.config, { enabled: advisor });
...
function backupState(paths) {
	const backup = path.join(
		paths.root,
		"backups",
		`${new Date().toISOString().replaceAll(/[:.]/gu, "-")}-${crypto.randomUUID().slice(0, 8)}`,
	);
...
function injectFailure(options, point) {
	if (options.failurePoint === point)
		fail(`injected init failure at ${point}`, "E_INJECTED_FAILURE");
}

function reconcile(config, advisor, options) {
	injectFailure(options, "reconcile");
	try {
		execFileSync(SCRIPT_PATH, ["reconcile", config, advisor], {
			encoding: "utf8",
			stdio: ["ignore", "pipe", "pipe"],
		});
	} catch (error) {
		const detail = error.stderr?.toString().trim() || error.message;
		fail(
			`gate configuration reconciliation failed: ${detail}`,
			"E_RECONCILE",
			error,
		);
	}
...
function writeArtifacts(preflightResult, options) {
	...
	if (!existingConfig)
		writeIfChanged(paths.config, options.configText ?? DEFAULT_CONFIG);
...
export function recordAdvisorOutcome(projectRoot, outcome) {
	...
	if (!fs.existsSync(manifestPath))
		fail(
			"cannot record Advisor outcome without an init runtime manifest",
			"E_ADVISOR_STATE",
		);
	if (!outcome || typeof outcome !== "object")
		fail("Advisor outcome must be an object", "E_ARGUMENT");
	const invoked = outcome.invoked === true;
	const decisionRecorded = outcome.decision_recorded === true;
	if (decisionRecorded && !invoked)
		fail(
			"a persisted Advisor decision requires an invocation",
			"E_ADVISOR_STATE",
		);
	if (invoked && outcome.logical_role_id !== "maister:advisor")
		fail(
			"Advisor invocation requires exact logical role maister:advisor",
			"E_ADVISOR_STATE",
		);
	if (typeof outcome.gate !== "string" || outcome.gate.length === 0)
		fail("Advisor outcome requires a gate", "E_ARGUMENT");
	let manifest;
	try {
		manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
	} catch (error) {
		fail(
			`cannot read init runtime manifest: ${error.message}`,
			"E_ADVISOR_STATE",
			error,
		);
	}
	...
	manifest.advisor.events.push({
		...
		retry_count: Number.isInteger(outcome.retry_count)
			? outcome.retry_count
			: 0,
...
export function runInit(options = {}) {
	...
	if (prepared.action === "cancel")
		return {
			...
			action: "cancel",
			advisor: {
				configured: prepared.advisor === "on",
				invoked: false,
				decision_recorded: false,
			},
			git: prepared.git,
	...
	if (prepared.action === "resume") {
		...
		try {
			previous = JSON.parse(
				fs.readFileSync(
					path.join(prepared.paths.root, "init-runtime-manifest.json"),
					"utf8",
				),
			);
		} catch (error) {
			fail(
				`cannot load init runtime manifest for resume: ${error.message}`,
				"E_RESUME",
				error,
			);
		}
		if (previous.transaction?.status !== "failed")
			fail("only a failed init transaction can be resumed", "E_RESUME");
	...
	try {
		...
		if (manifest.post_run_git.index !== prepared.git.index)
			fail("init changed the Git index", "E_GIT_MUTATION");
		persistManifest(prepared.paths, manifest);
		return {
			classification: prepared.classification,
		...
		fail(
			`init transaction failed: ${error.message}`,
			"E_INIT_TRANSACTION",
			error,
		);
	}
...
function cli(argv) {
	...
	let result;
	if (command === "record-advisor")
		result = recordAdvisorOutcome(options.projectRoot, options.outcome);
...
if (
	...
	try {
		...
		process.stderr.write(
			`${error.code ?? "E_INIT_RUNTIME"}: ${error.message}\n`,
		);
		process.exitCode = 1;
	}
}
