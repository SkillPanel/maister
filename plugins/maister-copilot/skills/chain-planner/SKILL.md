...
```
node ${MAISTER_PLUGIN_ROOT}/skills/umbrella/scripts/umbrella.mjs validate --root <root> --definition <draft>
```
...
| `/maister-chain-planner` | one ask_user for the task text, then as above |
...
**Exactly one question is ever asked, with ask_user, and only when the
