# Performance Orchestrator — prose twin

The prose phases of the `performance` workflow: the twin that `workflow-engine/workflows/performance.md` is
...
reference in it resolves from `skills/performance/`, never from `skills/performance/references/`.
...
# are its top-level siblings (the pro register § A1)
orchestrator:
  options:
