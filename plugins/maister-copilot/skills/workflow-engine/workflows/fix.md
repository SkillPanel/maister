# Fix workflow — node prose

The node-by-node companion to `fix.yml`. The engine executes every `direct:`
...
nodes exist, what they need and what they declare. It says what guards the fix
but not what proving a reproduction means, what the verification node runs, or
what finishing a dispatched run involves — and those are the things a reader of
...
reached as `workflow:fix` from a chain node that carries a `dir:`, and nowhere
else — so the prose below is written for a dispatched worker acting on a seed,
not for an operator who read a command's help.

**Its sibling is `change.md`.** This definition is that spine with the
...
the workflow's prose orchestrator; this definition has none, so for a fix run
...
phase it gates**, so `fix-approval` writes `verify`. A gate inherits rather than
...
**A skipped node still draws.** When `fix` is skipped its card renders with its
own hint and its skip reason, which is the point of guarding the node rather
...
node is skipped for a parent's benefit. **A fix run is never embedded.**

`research.yml` has an `## Embedded mode` section because it declares that input
and guards its final node on it. Nothing here does. The one guard in this
definition is `fix`, and what it reads is this run's own reproduction result,
not a parent's flag — so there is no embedded path to describe and none to be
written later without changing the definition first.
...
The node that proves the defect is real. The statement in `with:` is the whole
of what the work is; for this workflow it is the defect — what was expected, and
what happens instead.

**Trace before you test.** Find where the behaviour is produced, form a root
cause hypothesis with evidence — file paths, the code path the input takes — and
only then write the test. A test written against a guess reproduces the guess.

**Write the test that should fail.** In the project's own suite, following the
project's own conventions for where such a test lives: set up the conditions the
statement describes, assert the **correct** behaviour, and run it. It must fail,
and the failure must be the defect rather than a broken test.

**Run it and record what happened.** `red_proven` is `true` only when the test
...
> these conditions. Is the statement accurate, or are there conditions it does
> not name?
>
> **Default under a non-terminal driver** (`reproduction-not-red`): record the
> reproduction as not proven — `red_proven: false` — write the red-gate artifact
> saying exactly what was tried and what it did instead, and let the run carry
> that to the gate. Do not widen the conditions until something fails: a test
> tuned until it breaks proves the tuning, not the defect. `fix` is guarded on
> this value and is skipped, and the operator decides at the gate.

**The artifact is `implementation/tdd-red-gate.md`.** It opens with the artifact
summary block: `## TL;DR` of one to five non-empty lines, then the key decisions
and the open questions. Positional, before any other `## ` heading — a summary
block that appears after another section is not a summary block. After it: the
root cause hypothesis and its evidence, the test's location, the command that
runs it, and its output — the failure when red was proven, and what it did
instead when it was not.

> **SELF-CHECK**: before this node is marked completed, verify that
> `implementation/tdd-red-gate.md` exists, opens with that block, and that the
> value recorded for `red_proven` matches what the artifact says happened. If
> either is missing: **STOP. Do NOT proceed.** Re-drive the node with the gap
...
The node that makes the defect go away. It is **guarded on `red_proven`** and is
skipped when the reproduction was never proved — a fix for a defect nobody has
demonstrated is a change nothing measures. When it is skipped, record the skip
reason as the reproduction result and let the gate decide; do not work around
the guard by fixing anything here.
...
while writing. When a new area is reached mid-fix — a part of the codebase the
first read did not cover — read its standards before coding it rather than
after.

**Fix the cause the reproduction named**, not the symptom the test asserts.
A change that makes the test pass without addressing the traced cause is the
defect this workflow is least able to catch, because everything downstream of it
goes green.

**Prove the test green before this node is finished.** Run the test the
reproduction wrote; it must now pass. That is this node's own exit condition and
it is separate from `verify`, which runs the whole suite afterwards for
regressions.

**Bounded is the operative word.** This workflow exists because a chain needs a
target smaller than a full development run. A defect that turns out to need a
schema change, a decision between architectural approaches, or edits across
subsystems is not this workflow's work: record what was found, make no
speculative change, and say in the node summary what shape the work actually is
so the gate's context carries it to an operator. Growing the fix to fit the
discovery is the failure this paragraph exists to prevent.

**The artifact is `implementation/work-log.md`.** It records what changed and
why: the root cause as confirmed rather than as hypothesised, the files touched,
the decision behind each non-obvious edit, the red-to-green transition with the
command that showed it, and anything deliberately left alone. It is a log rather
than a summary document, so it carries no positional summary block — the summary
this node contributes is its node summary in state.

> **SELF-CHECK**: before this node is marked completed, verify that
> `implementation/work-log.md` exists and records the test passing. If it is
> missing: **STOP. Do NOT proceed.** Re-drive the node with the gap named in the
...
**Recovery budget**: three attempts. A fix that has not held after three is the
signal that the defect is larger than this workflow; record what was learned and
carry it to the gate rather than starting a fourth.
...
**The context this gate carries is what makes it answerable.** Whether the
reproduction was proved, the root cause as confirmed, the verification verdict,
and anything the nodes above left open — failures not fixed, scope the statement
turned out to exceed. A run whose `fix` node was skipped says so here first: an
operator answering from outside the session has only what the request document
says, and "the defect could not be reproduced" is the most important thing it
can carry.
