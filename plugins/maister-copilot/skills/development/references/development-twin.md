# Development Orchestrator — prose twin

The prose phases of the `development` workflow: the twin that `workflow-engine/workflows/development.md` is
measured against, and the path an install with no script runtime takes. Nothing here runs on the
engine path. It is read only when `MAISTER_WORKFLOW_PROSE` is set to a non-empty value, or when
Node is absent — the branch is settled once per run in `../SKILL.md`, which is the only file
that decides between the two.

**Every relative path below is anchored at the skill directory, one level up from this file.**
The body is unchanged from the SKILL.md it moved out of, so a `../orchestrator-framework/…`
reference in it resolves from `skills/development/`, never from `skills/development/references/`.
...
4. **Phase icons (§ 8)**: every phase row this workflow writes to `dashboard-data.js` carries an `icon_hint` from the enum of seven — `analysis`, `spec`, `plan`, `code`, `verify`, `docs`, `done`. Never invent a value; pick the closest (the pro register § A2). The field is itself optional — a row that carries none, as an engine-projected row may, falls back to the viewer's default.
