...
**A5's TL;DR bound is the second such conflict.** Two of the migration run's three artifacts opened with 8 and 6 non-blank TL;DR lines against a frozen 1–5, so they are absent from its fixture rather than reworded. Nothing instructs a writer to keep inside the bound — the subagent prompts require the block, not its length — so the rule currently holds only where a writer happens to be terse.
