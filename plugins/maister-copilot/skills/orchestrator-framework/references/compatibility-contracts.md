...
**The exactly-one rule above is in conflict with those orchestrators' own prose, and the fixtures do not show it.** Both the migration and the performance orchestrator direct a phase to write `task_context.clarifications_resolved` beside their own context block, so a compliant run of either produces a state this schema refuses. The sampled migration run did exactly that and had the two lines removed by hand to land; the sampled performance run escaped only by recording the same fact elsewhere. Every other fixture carrying `task_context` is a development run, where it *is* the per-workflow block, so neither sampled fixture should be read as proof that a compliant run of these two workflows validates. `invalid/orchestrator-state/two-context-blocks/` holds the unedited bytes and keeps the refusal failing out loud; it is the fixture that must go when the rule does. The rule and the prose disagree and one of them has to move; until that is decided, neither is changed.
