...
| C1 | `.maister/umbrella.yml` | engine (umbrella init) | `version: 1` | additive — the branch convention, coordination, tracker, git host, per-person member list, per-member `autonomy` and `defaults.provider` are such additions; `routing.tiers` reserved | `umbrella-manifest` | synthetic (6-member), invalid (member without path, person members not a list) |
| C2 | dispatch envelope | engine | `version: 1` | additive — the optional `statement` and `workspace_root` are two such additions; `ticket` resolves override → the run's `task.key` → null | `dispatch-envelope` | synthetic (envelope, worker-seed chain), invalid (bad `autonomy`) |
...
The performance and migration context shapes were derived from their orchestrators' own definitions, because no real run at or above the floor existed when the contract was written. Both are now pinned by runs sampled at 2.3.0-beta.15, and neither context shape changed; the synthetic fixtures stay beside them for the field values a run stopped at its first gate leaves null.

**The exactly-one rule stood; the two orchestrators' prose moved to meet it.** Both the migration and the performance orchestrator used to direct a phase to write `task_context.clarifications_resolved` beside their own context block, so a compliant run of either produced a state this schema refuses. Both prose twins, and both engine definitions, now record `clarifications_resolved` inside their own context block instead, which is where the rule has always put it. The sampled runs predate that correction: the migration one had the two lines removed by hand to land, and the performance one escaped only by recording the same fact elsewhere, so neither should be read as proof that a run of these two workflows validates of itself — the corrected prose and definitions are what makes that true. `invalid/orchestrator-state/two-context-blocks/` holds the unedited bytes and stays: the rule stands, so the refusal stands with it, failing out loud for any state that carries a second context block.

**A5's TL;DR bound is the second such conflict.** Two of the migration run's three artifacts opened with 8 and 6 non-blank TL;DR lines against a frozen 1–5, so they are absent from its run fixture rather than reworded, and they are held as `invalid/summary-blocks/run-written-tldr/` instead. Nothing instructs a writer to keep inside the bound — the Artifact Summary Contract the subagent prompts carry requires the block and its headings, not a length, and the number lives only here — so the rule currently holds where a writer happens to be terse rather than because anything holds it there. That fixture goes if the bound goes, and stays as the case that proves the writers were told if the bound moves into them.
...
**Whose work a pending gate stops.** The session that asked, and the gated run's own directory — nothing wider. A call is denied when **either** the caller is the run's recorded driver (the hook payload's session id equals `orchestrator.driver.session.id` — `session_id` on Claude, `sessionId` on Copilot, § H1) **or** the call writes inside that run's task directory; the allow-list below still applies to both. A call from another session, outside the gated run's directory, is allowed, and that includes elsewhere under `.maister/`: an umbrella exists to hold concurrent runs, and one open question is not a reason to freeze the others. Two cases keep the older, wider reach because the hook cannot tell whose gate it is looking at — a payload carrying no session id, and a run recording no `driver.session.id` (a terminal driver, for instance) — and there the run binds every session under the working directory, which is the fail-closed direction. The deny reason names the run and says which of the four bound the caller. The stop nudge and the liveness beacon are unscoped: they are informational and change nothing on disk.
...
| Allow response | silence — exit 0, no stdout — except for the plugin's own invocations, which answer `{hookSpecificOutput: {hookEventName, permissionDecision: "allow", permissionDecisionReason}}` | the same, `{permissionDecision: "allow", permissionDecisionReason}` — no envelope |
...
- **An allow is silence, with one exception: the plugin's own runtimes.** Every state change goes through the engine's writer, which is a shell call, so without the exception a terminal operator approves their own workflow several times per node. The hook answers `allow` for a call meeting all five conditions and for nothing else: a shell tool; a command carrying no `;`, `&&`, `||`, `&`, backtick, `$(`, `<` or `>`, and at most one pipe whose left stage is `echo` or `printf` with no substitution; `node` as the first token; a second token that, after the plugin-root variable is expanded from the environment in either spelling, resolves inside a directory carrying this plugin's `.claude-plugin/plugin.json` — and equals the declared root where one is set — and ends at `skills/workflow-engine/scripts/workflow.mjs` or `skills/umbrella/scripts/umbrella.mjs`; and a first non-flag argument that is a verb that script declares. The path is the boundary, never the file name. Anything else stays silent and reaches the caller's own permission rules unchanged, and **a pending gate is unaffected**: a bound call is denied before any allow is reached, so the shell stays shut on the engine too (§ E2, and the reason the suspend sequence is one call).
