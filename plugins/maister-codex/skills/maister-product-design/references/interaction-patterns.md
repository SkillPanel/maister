...
Guides interaction quality in the product-design orchestrator's interactive phases. Defines two cognitive modes, the iterative refinement loop, and user checkpoint option design.
...
```
Present complete draft → user checkpoint (approve / revise / rethink)
  → [revision] → present complete revised draft → user checkpoint (same options)
  → [after soft cap] → user checkpoint (approve current / one more revision / step back)
```

**Standard structured choices**: `Approve (Recommended)`, `Revise`, `Rethink`. Describe the likely revision aspects in the prompt. Codex automatically supplies a free-form `Other` option for explanation or a more specific change.

**Soft cap choices** (after iteration limit): `Approve current (Recommended)`, `One more revision`, `Step back`.
...
## User checkpoint option design

Options are not just UI -- they shape the conversation. Well-designed options anticipate what the user is likely thinking.

When `request_user_input` is available, use one short question with two or three mutually exclusive choices. Put the recommendation first and suffix its label with `(Recommended)`. Do not create an `Other` choice because Codex adds a free-form option automatically. When the tool is unavailable, render the same choices in one concise final-response question and pause.

### Exploration Mode Options

Structure: two or three topical choices; Codex provides the free-form escape hatch.

**Pattern**:
- 2-3 mutually exclusive topical choices that advance exploration in specific directions
- use `Need more info` as one of those choices when uncertainty is likely
- rely on Codex's automatic free-form option for explanation

**Example** (Phase 2 exploration):
```
- "The main problem is [user frustration with X]"
- "Actually, it's more about [business need Y]"
- "Both matter; prioritize [X]"
```
...
Structure: approve + revise + structural rethink.

**Pattern**:
- `Approve (Recommended)` first when the draft is ready
- `Revise` with likely aspects named in its description
- `Rethink` for a structural alternative
- rely on Codex's automatic free-form option for detailed feedback
...
**Always preserve an open-ended escape hatch**: `request_user_input` supplies it automatically. In the direct-question fallback, explicitly say that the user may answer with another preference.
...
Rich contextual prose BETWEEN user checkpoint calls is the actual design conversation. user checkpoint calls are punctuation marks -- they structure the conversation but do not replace it.
